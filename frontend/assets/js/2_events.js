...
// ── Auto calculate status from attending vs capacity ──
...
const mockEvents = [
  ...
  },
  {
    ...
    description: "An evening immersive workshop covering the full lifecycle of event production — from concept to curtain call. Perfect for aspiring event managers and entertainment professionals.",
    highlights: [
  ...
  },
  {
    ...
    agenda: {
      "Evening": [
        ...
        { time: "08:30 PM - 09:30 PM", title: "Live Performances — Act I & II", sub: "Main Stage", tag: "Performance" },
  ...
  },
  {
    ...
    agenda: {
      "Day 1": [
        ...
        { time: "01:00 PM - 02:30 PM", title: "Startup Pitch Battle — Semi-Finals", sub: "10 startups, 5 minutes each", tag: "Pitch" },
  ...
  },
  {
    ...
    description: "An advanced full-day workshop for marketing professionals ready to master the complete digital marketing stack — from technical SEO to influencer strategy and conversion optimization.",
    highlights: [
  ...
  },
  {
    ...
    date: "August 18, 2026", location: "Paññāsāstra University, Phnom Penh",
    ...
    venue: { name: "Paññāsāstra Tech Hall", address: "Phnom Penh, Cambodia", parking: true, accessibility: false }
  },
  {
  ...
  },
  {
    ...
    agenda: { day1: [
      { time: "08:00 AM - 09:00 AM", title: "Morning Market Tour", sub: "Psar Leu — sourcing fresh ingredients", tag: "Tour" },
    ...
    speakers: [
      { name: "Joannes Rivière", title: "Executive Chef", company: "Cuisine Wat Damnak" }
    ],
  ...
  // ── Entertainment (IDs 32-37) ──
  {
    ...
    agenda: { day1: [
      ...
      { time: "08:30 PM - 10:00 PM", title: "Late Night Jam Session", sub: "Open stage — all musicians welcome", tag: "Jam" }
    ]},
    speakers: [
  ...
  },
  {
    ...
    description: "An evening celebrating the rich traditions of Khmer performing arts — classical dance, shadow puppetry, and traditional music.",
  ...
  },
  {
    ...
    description: "Exploring modern approaches to language learning — from AI-assisted tools to immersive methodologies. Focus on Khmer, English, and Mandarin.",
  ...
  },
  {
    ...
    agenda: { day1: [
      { time: "06:00 PM - 07:00 PM", title: "Portfolio Wall & Arrival Drinks", sub: "Hang your work — meet others", tag: "Arrival" },
  ...
  },
  {
    ...
    agenda: { day1: [
      ...
      { time: "09:00 AM - 12:00 PM", title: "Group Stage Games", sub: "Round robin — all teams play", tag: "Competition" },
  ...
  },
  {
    ...
    agenda: { day1: [
      ...
      { time: "09:00 PM - 10:00 PM", title: "Main Event & Championship", sub: "Title fight — Pradal Serey", tag: "Championship" }
    ]},
    speakers: [
  ...
  },
  {
    ...
    agenda: { day1: [
      ...
      { time: "06:15 AM - 11:00 AM", title: "Gran Fondo Race", sub: "80km route — aim to finish by 11 AM", tag: "Race" },
...
function updateSeeMore() {
  ...
  const seeLessCount = document.getElementById("seeLessCount");
  if (!container) return;

  const remaining  = lastFilteredEvents.length - visibleCount;
  const canMore    = remaining > 0;
  const canLess    = visibleCount > INITIAL_COUNT;

  container.style.display = (canMore || canLess) ? "flex" : "none";

  if (seeMoreBtn)   seeMoreBtn.style.display   = canMore ? "inline-flex" : "none";
  if (seeMoreCount) seeMoreCount.textContent    = canMore ? "(" + remaining + " more)" : "";

  if (seeLessBtn)   seeLessBtn.style.display    = canLess ? "inline-flex" : "none";
  if (seeLessCount) {
    const willHide = Math.min(12, visibleCount - INITIAL_COUNT);
    seeLessCount.textContent = canLess ? "(" + (remaining + willHide) + " hidden)" : "";
  }
}
...
function showMore() {
  visibleCount += 12;
  renderEvents(lastFilteredEvents);
}

// ── Show less events (step back 12 at a time) ──
function showLess() {
  visibleCount = Math.max(INITIAL_COUNT, visibleCount - 12);
  renderEvents(lastFilteredEvents);
  requestAnimationFrame(() => {
    const btn = document.getElementById("seeMoreContainer");
    if (btn) btn.scrollIntoView({ behavior: "instant", block: "end" });
  });
}
...
function renderEvents(events) {
  ...
  if (events.length === 0) {
    grid.innerHTML = '<div class="no-events"><p>No events found.</p></div>';
    updateSeeMore();
    return;
  }
  ...
  const status = getStatus(e.attending, e.capacity);
  const delay = Math.min(i * 0.05, 0.6);
  return `
    <div class="event-card fade-up" style="animation-delay:${delay}s"
      onclick="viewEvent(${e.id})">
      <div class="event-card-img">
        <img src="${e.image}" alt="${e.title}"
          onerror="this.src='/assets/images/Sport%20event.jpg'" />
...
// Note: handleSearch() lives in 1_navbar.js (loaded first, shared across pages).
// It sets window.currentSearch, which applyFilters() below reads.

// ── Apply filters ──
function applyFilters() {
  visibleCount = 12;
  var search = (typeof window.currentSearch !== "undefined"
  ...
  if (currentCategory !== "All") {
    filtered = filtered.filter(function(e) {
      return e.category.toLowerCase() === currentCategory.toLowerCase();
    });
  }
...
function viewEvent(id) {
  ...
  const eventWithStatus = {
    ...event,
    status: getStatus(event.attending, event.capacity)
  };

  saveEventToStorage(eventWithStatus);
  window.location.href = "/pages/8_event-detail.html";
}

// ── Get event by ID ──
function getEventById(id) {
  return mockEvents.find(function(e) {
    return e.id === parseInt(id);
  }) || null;
}

// ── Save/load event from localStorage ──
...
function loadEventFromStorage() {
  var e = localStorage.getItem("erms_selected_event");
...
// Build a de-duplicated suggestion list: matching categories first, then events.
...
// Wrap the matched part of the text in <mark> for emphasis.
...
function renderSuggestions(query) {
  ...
  suggestionItems = buildSuggestions(query);
  suggestionIndex = -1;
  ...
  box.innerHTML = suggestionItems.map(function(s, i) {
    return `
      <li class="suggestion-item" data-index="${i}" id="suggestion-${i}"
...
function highlightActiveSuggestion() {
  ...
  box.querySelectorAll(".suggestion-item").forEach(function(el, i) {
    const on = i === suggestionIndex;
    el.classList.toggle("active", on);
    el.setAttribute("aria-selected", on ? "true" : "false");
    if (on && input) input.setAttribute("aria-activedescendant", el.id);
  });
}

// Apply a chosen suggestion: fill the input, filter the grid, scroll to results.
function chooseSuggestion(i) {
  const s = suggestionItems[i];
  if (!s) return;

  const input = document.getElementById("searchInput");
  if (input) input.value = s.text;

  window.currentSearch = s.text.toLowerCase();
  currentSearch = s.text.toLowerCase();
  applyFilters();
  closeSuggestions();
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Search input listeners for filtering (live input + Enter) live in 1_navbar.js.
  // Here we only add the autocomplete dropdown behavior.
  ...
  if (searchInput && box) {
    ...
    searchInput.addEventListener("keydown", function(e) {
      ...
      if (e.key === "ArrowDown") {
        ...
        if (suggestionIndex >= 0) { e.preventDefault(); chooseSuggestion(suggestionIndex); }
    ...
    box.addEventListener("click", function(e) {
      ...
      if (item) chooseSuggestion(parseInt(item.dataset.index, 10));
    });
